...
from backend.ocean_datasets import OceanDatasetManager, dataset_catalog
...
ocean_manager = OceanDatasetManager()
...
@app.get("/api/ocean/path")
def ocean_file_path(dataset_key: str, date: str):
    day = pd.to_datetime(date).date()
    path = ocean_manager.local_path(dataset_key, day)
    return {"dataset_key": dataset_key, "date": date, "local_path": str(path), "exists": path.exists()}


@app.post("/api/ocean/download")
def ocean_download(dataset_key: str, date: str, overwrite: bool = False):
    day = pd.to_datetime(date).date()
    path = ocean_manager.download_dataset_for_day(dataset_key, day, overwrite=overwrite)
    return {"dataset_key": dataset_key, "date": date, "saved_to": str(path)}
